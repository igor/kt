...
export function statsCommand(): Command {
  return new Command('stats')
    ...
    .action((options) => {
      ...
      const nsSql = nsf ? ` AND ${nsf.sql}` : '';
      const nsParams = nsf ? nsf.params : [];

      const total = (db.prepare(`SELECT COUNT(*) as c FROM nodes WHERE 1=1${nsSql}`).get(...nsParams) as any).c;
      const active = (db.prepare(`SELECT COUNT(*) as c FROM nodes WHERE status = 'active'${nsSql}`).get(...nsParams) as any).c;
      const stale = (db.prepare(`SELECT COUNT(*) as c FROM nodes WHERE status = 'stale'${nsSql}`).get(...nsParams) as any).c;
      const compacted = (db.prepare(`SELECT COUNT(*) as c FROM nodes WHERE status = 'compacted'${nsSql}`).get(...nsParams) as any).c;
      const pending = (db.prepare(`SELECT COUNT(*) as c FROM nodes WHERE embedding_pending = 1${nsSql}`).get(...nsParams) as any).c;
      const summaries = (db.prepare(`SELECT COUNT(*) as c FROM nodes WHERE source_type = 'compaction'${nsSql}`).get(...nsParams) as any).c;

      const oldest = db.prepare(
        `SELECT updated_at FROM nodes WHERE status = 'active'${nsSql} ORDER BY updated_at ASC LIMIT 1`
